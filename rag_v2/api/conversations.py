"""
Persistência de conversas e mensagens para a RAG v2 em SQLite.
Banco isolado da V1 (conversations_v2.db).
"""
import json
import logging
import sqlite3
...
try:
    from core.config import CONVERSATIONS_DB_PATH, CONVERSATION_TITLE_MAX_CHARS
except ImportError:
    from ..core.config import CONVERSATIONS_DB_PATH, CONVERSATION_TITLE_MAX_CHARS
...
def init_db() -> None:
    """Inicializa as tabelas de conversas da v2."""
    ...
    try:
        ...
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_conversations_v2_user "
            "ON conversations(username)"
        )
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_messages_v2_conv "
            "ON messages(conversation_id)"
        )
        conn.commit()
        logger.info("Banco de conversas v2 pronto em %s", CONVERSATIONS_DB_PATH)
    finally:
        conn.close()
...
def create_conversation(username: str, title: str | None = None, model: str | None = None) -> dict:
    ...
    try:
        conn.execute(
            "INSERT INTO conversations (id, username, title, model, created_at, updated_at) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (conversation_id, username, title, model, now, now),
        )
        conn.commit()
    finally:
        conn.close()
