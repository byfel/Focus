...
"""
    ...
    try:
        ...
        logger.warning(
            "Falha no Query Expansion, continuando somente com a "
            "pergunta original.",
            exc_info=True,
        )
        return [query]
...
def search_qdrant(query, top_k):
    try:
        ...
        logger.error(
            "Não foi possível gerar embedding para a query %r.", query,
            exc_info=True,
        )
        return []
    ...
    try:
        result = client.query_points(
            ...
            # .tolist(): o qdrant-client espera uma lista de floats,
            # não um numpy.ndarray. Funcionava antes por coincidência
            # de compatibilidade — agora é explícito.
            query=query_embedding.tolist(),
            limit=top_k,
            with_payload=True,
        )
    except Exception:
        logger.error("Falha na consulta ao Qdrant para %r.", query, exc_info=True)
        return []
...
def retrieve(query, top_k=TOP_K, threshold=THRESHOLD):
    ...
    logger.debug(
        ...
        len(queries), query, queries[1:],
    )

    all_candidates = {}

    for current_query in queries:
        results = search_qdrant(current_query, EXPANSION_TOP_K)
        logger.debug(
            "Busca Qdrant %r -> %d resultado(s)", current_query, len(results)
        )
    ...
    for item in candidates:
        item["rrf_score"] = calculate_rrf_score(item["rrf_ranks"])

    # --------------------------------------------------------
    # Threshold: usa o melhor score original do Qdrant, não o RRF.
    # --------------------------------------------------------
    filtered = [item for item in candidates if item["best_score"] >= threshold]

    if not filtered:
        logger.info(
            "Nenhum candidato passou do threshold %.4f para %r", threshold, query
        )
        return []

    # NOTA IMPORTANTE: MIN_DOMINANCE_RATIO (config.py) não é usado
    # aqui. Existia no config original sem estar implementado em
    # lugar nenhum. Não adicionei uma versão "adivinhada" dessa
    # lógica — isso mudaria quais respostas o usuário recebe, e
    # prefiro confirmar com você a intenção original (ex.: só aceitar
    # o melhor resultado se ele dominar os demais por essa razão?)
    # antes de ativar qualquer filtro adicional aqui.

    # --------------------------------------------------------
    # Ordenação: RRF, com desempate por aparições, melhor score,
    # melhor posição.
    # --------------------------------------------------------
    filtered = sorted(
        filtered,
    ...
    selected = ranked[:FINAL_CONTEXTS]

    for index, item in enumerate(selected, start=1):
        logger.debug(
            "%d. RRF=%.6f Score=%.4f Aparições=%d %s p.%s chunk=%s",
            index,
            item["rrf_score"],
            item["best_score"],
            item["appearances"],
